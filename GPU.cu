    ...
    unsigned int ** dev_nCells)
{
    ...
    *nNonEmptyCells = numNonEmptyCells;
    ...
    errCode = cudaMemcpy((*dev_index), (const void*)index, sizeof(struct grid) * (*nNonEmptyCells), cudaMemcpyHostToDevice);
	if(errCode != cudaSuccess)
    {
    ...
    errCode = cudaMemcpy((*dev_gridCellLookupArr), (const void *)gridCellLookupArr, sizeof(struct gridCellLookup) * (*nNonEmptyCells), cudaMemcpyHostToDevice);
	if(errCode != cudaSuccess)
    {
