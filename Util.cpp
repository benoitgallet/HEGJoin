...
void Util::egoJoinPreQueue(pPoint A, int frA, int toA, pPoint B, int frB, int toB, int start_dim, std::list<unsigned int> * result, unsigned int * nbNeighbors)
{
	...
	if (A_sz < MINLEN && B_sz < MINLEN)
	{
		simpleJoinPreQueue2(A, frA, toA, B, frB, toB, i, result, nbNeighbors);
	...
	{
        // printf("\nCase 2 (self-thread)\n");
		egoJoinPreQueue(A, frA             , frA + A_sz / 2, B, frB             , frB + B_sz/2, start_dim, result, nbNeighbors); // f f
		egoJoinPreQueue(A, frA             , frA + A_sz / 2, B, frB + B_sz/2 + 1, toB         , start_dim, result, nbNeighbors); // f s
		egoJoinPreQueue(A, frA + A_sz/2 + 1, toA           , B, frB             , frB + B_sz/2, start_dim, result, nbNeighbors); // s f
		egoJoinPreQueue(A, frA + A_sz/2 + 1, toA           , B, frB + B_sz/2 + 1, toB         , start_dim, result, nbNeighbors); // f s
		return;
	}
	...
	{
        // printf("\nCase 3 (self-thread)\n");
		egoJoinPreQueue(A, frA             , frA + A_sz / 2, B, frB, toB, start_dim, result, nbNeighbors); // f full
		egoJoinPreQueue(A, frA + A_sz/2 + 1, toA           , B, frB, toB, start_dim, result, nbNeighbors); // s full
		return;
	}
	...
	{
        // printf("\nCase 4 (self-thread)\n");
		egoJoinPreQueue(A, frA, toA, B, frB             , frB + B_sz/2, start_dim, result, nbNeighbors); // f f
		egoJoinPreQueue(A, frA, toA, B, frB + B_sz/2 + 1, toB         , start_dim, result, nbNeighbors); // f s
		return;
	}
}
...
void Util::simpleJoinPreQueue2(pPoint A, int frA, int toA, pPoint B, int frB, int toB, int m, std::list<unsigned int> * result, unsigned int * nbNeighbors)
{
	...
	if (r3_end == -1) // 1 or 2 intervals
    {
        if (r2_end == -1) // 1 interval
        {
            simpleJoinPreQueue(A, frA, toA, B, frB, toB, result, nbNeighbors);
            return;
        }
